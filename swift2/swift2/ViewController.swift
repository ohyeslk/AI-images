...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, AVAudioPlayerDelegate, UIWebViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSTimer.scheduledTimerWithTimeInterval(self.update_frequency, target: self, selector: #selector(ViewController.captureAndSendFrame(_:)), userInfo: nil, repeats: true)
...
//        }
    ...
    @IBAction func captureAndSendFrame(sender: UIButton?) {
        ...
        if waitingForCaptions {
            if !captionsCued {
                captionsCued = true
                NSTimer.scheduledTimerWithTimeInterval(2, target: self, selector: #selector(ViewController.captureAndSendFrame(_:)), userInfo: nil, repeats: false)
            }
